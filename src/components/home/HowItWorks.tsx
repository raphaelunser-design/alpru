...
export default function HowItWorks() {
  return (
    <section id="so-funktionierts" className="relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] w-screen bg-white text-slate-950">
      <div className="mx-auto w-full max-w-6xl px-4 pb-16 md:px-6 md:pb-20">
        ...
        <div className="relative mt-12 grid gap-4 md:grid-cols-5 md:gap-0">
          <div className="absolute left-[10%] right-[10%] top-7 hidden h-px bg-gradient-to-r from-sky-200 via-sky-400 to-emerald-300 md:block" />
          {steps.map(([number, title, text], index) => (
            <Reveal key={number} delay={index * 0.05}>
              <article className="relative grid gap-4 rounded-[1.25rem] border border-slate-200 bg-white p-5 shadow-[0_18px_55px_rgba(15,23,42,0.07)] md:border-transparent md:bg-transparent md:p-3 md:shadow-none">
                <div className="flex items-center gap-4 md:flex-col md:text-center">
                  <div className="relative z-10 grid h-14 w-14 shrink-0 place-items-center rounded-full border-4 border-white bg-sky-500 text-white shadow-[0_16px_36px_rgba(14,165,233,0.26)]">
                    <StepIcon index={index} />
                  </div>
                  <div className="min-w-0">
                    <div className="text-xs font-extrabold uppercase tracking-[0.16em] text-sky-700">{number}</div>
                    <h3 className="mt-1 text-base font-extrabold leading-tight text-slate-950">{title}</h3>
                  </div>
                </div>
                <p className="border-l border-sky-100 pl-4 text-sm leading-6 text-slate-600 md:border-l-0 md:pl-0 md:text-center">
                  {text}
                </p>
              </article>
            </Reveal>
